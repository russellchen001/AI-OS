...
Microsoft Word real-E2E blocker (2026-09-01): the installed Word scripting dictionary exposes document/text/save/PDF operations, and deterministic object creation plus an isolated activated save probe succeeded. The in-process adapter prototype correctly stages files through Word's container and never targets `active document` for closure, but repeated create/read-back E2E runs still block in Word `save as` with no visible window or sheet and eventually return AppleEvent timeout `-1712`. This milestone is not accepted or committed. More runtime evidence from Word is required before continuing that adapter; Office remains In Progress and P15 remains 5 of 11.
