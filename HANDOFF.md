...
Microsoft Excel Phase A + Phase B mutation (2026-09-01): provider-neutral `spreadsheet.edit` routes to the deterministic Microsoft Excel adapter with one-time confirmation. Phase A real landed-XLSX E2E passes bounded typed `set_cell`, bounded `set_formula` with formula/calculated-value read-back, `clear_cell`, unique XLSX save-copy, no-overwrite, original preservation, fresh post-save ownership, output reopen validation, workbook/window restoration, Excel process preservation, and user-workbook preservation. The Phase B mutation subphase adds deterministic `add_worksheet`, `rename_worksheet`, `delete_worksheet`, and explicit named-sheet edits. Excel 16.78 does not guarantee a new worksheet's insertion index, so AddWorksheet snapshots worksheet identities before creation, performs the proven `make new worksheet at freshWorkbook`, reacquires the workbook, computes the unique set difference, and only then renames/uses that identity. Delete refuses the final worksheet, verifies other worksheets remain present, and uses no UI/System Events automation. Reopen validation compares the final worksheet identity set so temporary pre-rename/pre-delete names are not falsely required to survive. Full Excel Phase B remains **In Progress** pending worksheet list/count, specific-sheet bounded read, and multi-sheet read acceptance; Excel Common Capability then continues with row/column, formatting, sort/filter, chart integration, and the final realistic workflow. Office remains **In Progress** and P15 remains **5 of 11**.
