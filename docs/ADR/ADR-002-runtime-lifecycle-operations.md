...
The final status-coordinator correction uses one shared drain-cycle Promise. Every manual, periodic, terminal, Bulk, or endpoint-change caller joining an active cycle waits until all coalesced latest-generation work finishes. Requests during one physical query mark a single next iteration; query completion and the endpoint effect share the same generation marker and cannot double-schedule that generation. Stale successes are ignored, and stale failures neither set current status error nor reject a cycle that later succeeds. Only a final applicable failure preserves prior status, sets the fixed error, and rejects the shared Promise.
