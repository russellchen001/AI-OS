...
Ollama and Open WebUI require explicit HTTP or HTTPS endpoints. Localhost and loopback addresses are local; other valid hosts are remote. Embedded credentials, hostless URLs, and non-network or executable schemes are rejected. OpenClaw derives its gateway exclusively from the active native profile rather than accepting a request override. Gateway validation accepts `ws`, `wss`, `http`, and `https`; browser opening uses a typed `ws` to `http` or `wss` to `https` scheme conversion that preserves the validated host, port, path, and query.
...
Context collection is runtime-specific: OpenClaw configuration is read only for OpenClaw, Docker container inspection occurs only for Open WebUI management, and open-only plans do not collect unrelated lifecycle ownership facts. An unloaded OpenClaw service freezes ordered `bootstrap` then `kickstart` commands; a loaded service freezes only `kickstart`. OpenClaw verification combines the stable launch-service observation with the existing structured connection snapshot and never curls a WebSocket URL.

`prepare_execution_plan` is the sole M1B2B-facing preparation boundary. It validates one request, performs only action-specific collection, freezes that context, and builds the typed plan atomically. Callers cannot supply a separately collected context. OpenClaw start verification retains a private, non-serializable frozen profile identity, endpoint, and secret; its redacted debug representation cannot reveal the token. Verification performs a fresh bounded connection test against that accepted profile and never rereads the active profile or trusts historical connection state. OpenClaw open collects only the gateway/browser endpoint, and an already-unloaded stop becomes a verified no-op.
...
Open WebUI container operations first establish the installed Docker Desktop `desktop-linux` context as the expected local Unix socket, then freeze that socket into every `docker info`, `ps`, `inspect`, `start`, `stop`, and `restart` command. Docker environment selectors are removed per child process, so caller-selected remote hosts or contexts cannot affect execution. Docker command timeouts and spawn or inspection failures remain typed inspection failures and cannot prove that a container is missing or stopped.
