    ...
    -> Vec<McpServer>
{
    vec![
        McpServer {
            ...
            description:
                "Read and manage approved local files."
                    .to_string(),
    ...
    -> Result<Vec<McpServer>, String>
{
    ...
    .map_err(|error| {
        format!(
            "Unable to parse MCP configuration: {}",
            error
        )
    })
}
...
fn write_servers(
    ...
    .map_err(|error| {
        format!(
            "Unable to create MCP configuration directory: {}",
            error
        )
    })?;
    ...
    let contents =
        ...
        .map_err(|error| {
            format!(
                "Unable to serialize MCP configuration: {}",
                error
            )
        })?;
...
fn validate_input(
    ...
    {
        return Err(
            "A URL is required for HTTP and SSE MCP servers."
                .to_string(),
        );
    }
