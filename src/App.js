...
function App() {
    ...
    const handleMessage = useCallback((nextMessage) => {
        setMessage(nextMessage);
    }, []);
    const { settings, updateSetting, mergeSettings, resetSettings, } = useSettings(handleMessage);
    const { metrics, refreshMetrics, } = useMetrics();
    const { services, lastUpdated, isChecking, globalAction, serviceAction, openAction, isBusy, runningCount, stoppedCount, unknownCount, allRunning, healthCheck, startService, stopService, openService, handleGlobalToggle, } = useServices({
        settings,
    ...
    const backup = useBackup({
        settings,
        onMessage: handleMessage,
        onSettingsRestored: mergeSettings,
    });
    const logs = useLogs({
        lineLimit: settings.logLineLimit,
    ...
    return (_jsxs("div", { style: appStyle, children: [_jsx(Sidebar, { activePage: activePage, settings: settings, onPageChange: setActivePage }), _jsxs("main", { className: "main-content", children: [_jsx(Header, { isChecking: isChecking, lastUpdated: lastUpdated }), activePage ===
                        ...
                        "Models" && (_jsx(ModelsPage, { models: models
                            ...
                            .setSearchText, onRefresh: models
