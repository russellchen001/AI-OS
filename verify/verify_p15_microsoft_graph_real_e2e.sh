...
for variable in \
  AI_OS_GRAPH_E2E_DRIVE_ID \
  AI_OS_GRAPH_E2E_ITEM_ID \
  AI_OS_GRAPH_E2E_WORKSHEET \
  AI_OS_GRAPH_E2E_RANGE
do
  if [[ -z "${!variable:-}" ]]; then
    if [[ -n "$missing" ]]; then
...
if [[ -n "$missing" ]]; then
  echo "SKIP $name: TEST_FIXTURE_REQUIRED missing=$missing"
  exit 0
fi
...
output="$(
  cargo test \
    ...
    microsoft_graph::tests::microsoft_graph_real_e2e \
